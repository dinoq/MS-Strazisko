...
import { DBObject, DetailFrameDef, FormDef } from "../../../constants/types";
                    ...
                    if (item.editable) {
                        if (item.componentType == ComponentType.INPUT) {
                            return (
                                <div key={"input-" + i}>
                                    <div className="d-flex justify-content-center">
                                        <input type={item.inputType ? item.inputType : "text"} placeholder={props.DBObject.attributes[item.attributeKey]} value={(props.DBObject.editedAttrs[item.attributeKey])?props.DBObject.editedAttrs[item.attributeKey]:props.DBObject.attributes[item.attributeKey]} onChange={props.updateDBObject.bind(this, item.attributeKey)} required/>
                                    </div>
                                </div>
                            );
                            ...
                            if (props.mode == DetailFrameMode.EDITING_ENTRY) {
                                return (
                                    <select key={"selectbox-" + i} value={(props.DBObject.editedAttrs[item.attributeKey])?props.DBObject.editedAttrs[item.attributeKey]:props.DBObject.attributes[item.attributeKey]} onChange={props.updateDBObject.bind(this, item.attributeKey)} disabled={!item.editable}>
