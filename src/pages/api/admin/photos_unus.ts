...
const handler = async (req, res) => {
  ...
  if (req.method == "GET") {
    ...
    const stmt = db.prepare(
      "SELECT id_photo as id, filename FROM photos where id_album=" +
        albumID +
        ";"
    );
    ...
    try {
      const stmt = db.prepare(
        "UPDATE albumPasswords SET passwordHash = ? WHERE id_albumPasswords = ?"
      );
      ...
      return res
        .status(500)
        .send("ERROR! " + (process.env.NODE_ENV === "production" ? "" : error));
    }
    db.close();
    return res.status(200).send("Success!");
    ...
    try {
      ...
      if (sqlResults.length) {
        // Nemuze se smazat rok, pokud obsahuje alba
        db.close();
        return res
          .status(500)
          .send(
            "Chyba! Daný rok obsahuje nějaká alba. Nejprve musíte smazat je a až potom samotný školní rok!"
          );
      }
